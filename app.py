...
def main():
    ...
    if st.button("Generate DOCX"):
        try:
            ...
            t1 = left.add_table(rows=1, cols=len(df_positions.columns)); set_table_exact_width(t1, 3.45); t1.autofit=False
            ...
            set_col_widths(t1, [0.5,1.0,0.5,0.85,0.8,0.8])

            left.add_paragraph("Vimshottari Mahadasha").runs[0].bold=True
            t2 = left.add_table(rows=1, cols=len(df_md.columns)); set_table_exact_width(t2, 3.45); t2.autofit=False
            ...
            t3 = left.add_table(rows=1, cols=len(df_ap.columns)); set_table_exact_width(t3, 3.45); t3.autofit=False
            ...
            p2.add_run().add_picture(img_nav, width=Inches(3.25))
