...
def _build_kundali_docx_entry(name, dob, tob, place, tz_override):
    # Prefer user's own generator if present
    for fn_name in ("generate_kundali_docx_full", "generate_kundali_docx", "create_docx"):
        fn = globals().get(fn_name)
        if callable(fn):
            return fn(name, dob, tob, place, tz_override)
    # If nothing explicit, try a fallback named main_build_doc if user added it
    fn = globals().get("main_build_doc")
    if callable(fn):
        return fn(name, dob, tob, place, tz_override)
    return None
# === End helpers ===
