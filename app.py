import os, re, datetime, json, urllib.parse, urllib.request
...
st.set_page_config(page_title="Kundali — Single‑Page Report", layout="wide", page_icon="🪔")
...
def render_north_diamond_blank(size_px=900, stroke=3):
...
# ----------------- App -----------------
def main():
    st.title("Single‑Page Kundali Report (North‑Indian)")
    ...
    if st.button("Generate"):
        try:
            ...
            df_md = pd.DataFrame([{"Planet": HN[s["planet"]], "End": s["end"].strftime("%d-%m-%Y"), "Age": round(((s["end"] - dt_local).days / YEAR_DAYS), 1)} for s in md_segments])
            now_local = datetime.datetime.now()
            rows_ap = next_ant_praty_in_days(now_local, md_segments, days_window=2*365)
            df_ap = pd.DataFrame([{"Major": HN[r["major"]], "Antar": HN[r["antar"]], "Pratyantar": HN[r["pratyantar"]], "End": r["end"].strftime("%d-%m-%Y")} for r in rows_ap])

            # Charts (blank diamond grids)
            img_lagna = render_north_diamond_blank(size_px=900, stroke=3)
            img_nav   = render_north_diamond_blank(size_px=900, stroke=3)

            # ---- DOCX Single‑Page Layout ----
            doc = Document()
            sec = doc.sections[0]
            ...
            title.runs[0].font.size = Pt(14); title.runs[0].bold = True

            # 2‑column table to control layout
            layout = doc.add_table(rows=1, cols=2)
            layout.autofit = True
            layout.columns[0].width = Inches(3.9)
            layout.columns[1].width = Inches(3.9)

            # LEFT CELL: details + tables (condensed)
            ...
            add_table_borders(t1, size=6); set_table_font(t1, pt=9)
            ...
            for _,row in df_md.iterrows():
                r=t2.add_row().cells
                for i,c in enumerate(row): r[i].text=str(c)
            add_table_borders(t2, size=6); set_table_font(t2, pt=9)
            ...
            for _,row in df_ap.iterrows():
                r=t3.add_row().cells
                for i,c in enumerate(row): r[i].text=str(c)
            add_table_borders(t3, size=6); set_table_font(t3, pt=9)

            # RIGHT CELL: two charts stacked with captions
            right = layout.rows[0].cells[1]
            p = right.add_paragraph("Lagna Kundali (North‑Indian)"); p.runs[0].bold = True
            img_lagna.seek(0); right.add_paragraph().add_run().add_picture(img_lagna, width=Inches(3.6))

            p = right.add_paragraph("Navamsa Kundali (D‑9)"); p.runs[0].bold = True
            img_nav.seek(0); right.add_paragraph().add_run().add_picture(img_nav, width=Inches(3.6))

            # Export with requested filename
            fname = sanitize_filename(name) + ".docx"
            bio = BytesIO(); doc.save(bio); bio.seek(0)

            st.success("Report generated.")
            st.download_button("⬇️ Download Single‑Page DOCX", bio.getvalue(), file_name=fname)

            # On‑screen preview: left tables + right blank grids
            ...
            with rc:
                st.subheader("Lagna Kundali (Blank Grid)"); st.image(img_lagna, use_container_width=True)
                st.subheader("Navamsa Kundali (Blank Grid)"); st.image(img_nav, use_container_width=True)
