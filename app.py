...
def render_chart_png_bytes(sign_by_house, planets_by_house, size=800):
    """Return PNG bytes with background and overlay."""
    from PIL import Image, ImageDraw, ImageFont
    import io, base64
    bg_bytes = base64.b64decode(BG_PNG_B64)
    ...
    font_sign = pick_font(int(size*0.05))
    font_planet = pick_font(int(size*0.045))

    for h,(rx,ry) in REL_CENTERS.items():
        x = int(rx*size); y = int(ry*size)
        ...
        pw, ph = draw.textbbox((0,0), pls, font=font_planet)[2:4] if pls else (0,0)
        draw.text((x - sw//2, y - sh//2 - 6), sign, fill=(0,0,0), font=font_sign)
        if pls:
            draw.text((x - pw//2, y + sh//2), pls, fill=(0,0,0), font=font_planet)
